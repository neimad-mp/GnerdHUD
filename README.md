...
This README reflects the 0.5.x work (segmented arcs). **0.5.5 adds Center Cast & Mirror bars with per-bar height/width controls and a tabbed Options panel.**
...
- Options → **Center Bars** tab
  - Enable Cast Bar / Enable Mirror Bar
  - **Cast Scale (width) • Cast Height (px)**
  - **Mirror Scale (width) • Mirror Height (px)**
...
Options panel
- **Tabbed layout:** **Global** and **Center Bars** (each has its own scroll area and hover hints).
- Global tab:
...
- ESC and [Close] to dismiss; panel is draggable.
